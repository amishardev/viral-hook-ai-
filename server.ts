...
// Ensure DNS resolution works correctly in typical sandbox envs
...
// Set higher limits for parsing base64 storyboard canvases
  ...
  try {
    ...
    if (!finalApiKey || finalApiKey === "MY_GEMINI_API_KEY") {
      return res.status(400).json({
        error: "Gemini API key is not configured. Please add your key in the top-right Settings panel to run analyses."
      });
    }

    if (!storyboard) {
      return res.status(400).json({
        error: "Storyboard image payload is required."
      });
    }

    // Initialize @google/genai cleanly as per SDK directions
    ...
    // Strip image URL headers if they exist ("data:image/png;base64,...")
    const cleanBase64 = storyboard.replace(/^data:image\/\w+;base64,/, "");

    // Prepare contents
    const contents: any[] = [];

    // The first user message contains the storyboard vision element + full system context & prompt
    ...
    // If there is subsequent chat history, map and push them to represent the full conversational transcript
    // Since contents[0] is already the first User turn (containing storyboard + system prompt + initial request),
    // we must skip any consecutive User turns in history to ensure strictly alternating roles.
    if (Array.isArray(history) && history.length > 0) {
      let lastRole = "user"; // contents[0] has role: "user"
      for (const turn of history) {
        const currentRole = turn.role === "user" ? "user" : "model";
        if (currentRole === lastRole) {
          // Skip consecutive duplicates of the same role
          continue;
        }
        contents.push({
          role: currentRole,
    ...
    // Call Gemini 3.5 Flash (vision & reasoning)
    const response = await ai.models.generateContent({
      model: "gemini-3.5-flash",
      contents: contents, // Sending complete conversation array
      config: {
    ...
    // Extract JSON string from raw text using the robust helper, removing markdown wrappers or preambles
    ...
    try {
      ...
      return res.status(500).json({
        error: "Failed to parse system response as JSON. Please try refining your video context or resubmitting.",
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Viral AI server running at http://localhost:${PORT}`);
  });
}
